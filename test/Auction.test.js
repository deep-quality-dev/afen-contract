const AFENERC1155Token = artifacts.require("AFENERC1155Token");
const AFENToken = artifacts.require("AFENToken");
...
contract("ERC1155Auction", (accounts) => {
    ...
    before(async () => {
        await AFENERC1155Token.new(
            "AFEN1155",
            "AFEN1155",
            accounts[1], // signer
            "https://api-testnet.afen.co/contractMetadata/{address}", // contractURI
            "AFEN_", // tokenURIPrefix
            "Afen__", // uri
            { from: accounts[0] }
        ).then(function (instance) {
            afenerc1155token_contract = instance;
        });

        await AFENToken.new({ from: accounts[0] }).then(function (instance) {
            afentoken_contract = instance;
        });
        ...
        await servicefee_contract.setAFENTokenContract(afentoken_contract.address, { from: accounts[0] })

        await ERC1155AuctionMock.new(
            afenerc1155token_contract.address,
    ...
    const mint = async (minter, tokenId, supply = 1, fees = []) => {
        ...
        await afenerc1155token_contract.mint(
            tokenId,
    ...
    describe("auction with single token", () => {
        ...
        it("creating auction not working if caller is not the owner of the token", async () => {
            await afenerc1155token_contract.setApprovalForAll(
        ...
        it("result auction", async () => {
            ...
            assert.equal(await afenerc1155token_contract.balanceOf(accounts[9], tokenId), 1);
    ...
    describe("auction with multiple tokens", () => {
        it("working with multiple tokens", async () => {
            ...
            const saleContractBuySignature = soliditySha3(
                afenerc1155token_contract.address, // token
                tokenId, // tokenId
            ...
            await sale_contract.buy(
                afenerc1155token_contract.address, //IERC1155 token,
                tokenId, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (AFENERC1155Token)
            ...
            //Token transfered correctly
            assert.equal(await afenerc1155token_contract.balanceOf(accounts[6], tokenId), 1);
            ...
            //Token transfered correctly
            assert.equal(await afenerc1155token_contract.balanceOf(accounts[8], tokenId), 1);
