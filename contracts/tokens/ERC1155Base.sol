...
abstract contract ERC1155Base is
    ...
    constructor(string memory contractURI, string memory tokenURIPrefix, string memory _uri) HasContractURI(contractURI) ERC1155Metadata_URI(tokenURIPrefix) ERC1155(_uri) {
    ...
    function _mint(uint256 _id, Fee[] memory _fees, uint256 _supply, string memory _uri) internal {
        ...
        for (uint256 i = 0; i < _fees.length; i++) {
            sumFeeBps = sumFeeBps + _fees[i].value;
        }
     ...
     */
    function _setTokenURI(uint256 tokenId, string memory _uri) override virtual internal {
    ...
    // function uri(uint256 _id) override(ERC1155Metadata_URI, ERC1155) virtual external view returns (string memory) {
    //     return _tokenURI(_id);
    // }
    function uri(uint256 _id) override(ERC1155Metadata_URI, ERC1155) public virtual view returns (string memory) {
        return _tokenURI(_id);
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155, ERC165Storage, IERC165) returns (bool) {
